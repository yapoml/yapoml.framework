...
namespace Yapoml.Test.Workspace
{
    internal class WorkspaceTests
    {
        ...
        public void SetUp()
        {
            ...
            _parser.Setup((cp) => cp.ParsePage(It.IsAny<string>())).Returns(new Yapoml.Workspace.Parsers.Yaml.Pocos.Page());
            _parser.Setup((cp) => cp.ParseComponent(It.IsAny<string>())).Returns(new Yapoml.Workspace.Parsers.Yaml.Pocos.Component());
        }
